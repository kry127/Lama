  ...
  struct
    ...
    *)
    ...
    module Typecheck = struct
      ...
      module Conformity =
        struct
          ...
          (* check that in program state 'conf' value 'value' can be used as type 't': "value conforms typ" *)
          let rec value_conforms conf value typ
            = match (value, typ) with
            | (_   , TAny) -> true
            | (Value.Cast(_, _, tt, _, _), tt') -> conforms tt tt'
            | (Value.Empty , TVoid) -> true
            | (Value.Var (Global name), _) -> value_conforms conf (State.eval conf name) typ
            | (Value.Var _, _) -> report_error "only global vars are supported at interpretation"
            | (Value.Elem(container, i), l) -> (
              match container with
              | Value.Sexp(_, args)
              | Value.Array(args) -> value_conforms conf (args.(i)) l
              | Value.String _    -> l == TConst
              | _                 -> false
            )
            | (Value.Int _, TConst) -> true
            | (Value.String _, TString) -> true
            | (Value.Array arr, TArr(tarrElem)) -> Array.for_all (fun arrElem -> value_conforms conf arrElem tarrElem) arr
            | (Value.Sexp (name, vals), TSexp(tname, tvals)) ->
                String.equal name tname && List.for_all2 (value_conforms conf) (Array.to_list vals) tvals
            ...
            | (_, TUnion types) -> List.exists (fun tt -> value_conforms conf value tt) types
            | _, _ -> false
      ...
      let type_check ctx expr =
        let rec type_check_int ret_ht ctx expr
          =
            ...
            match expr with
            | Cast(_, _, t, _, _)      -> t, expr (* Trivial rule, but does it preserve soundness? It cannot be generated by parser. *)
            | Const _                  -> TConst, expr
            ...
            | Elem (l, arr, index)     ->
                                       let t_arr, e_arr  = type_check_int ret_ht ctx arr in
                                       ...
                                       then
                                         ...
                                         in
                                         match t_arr with
                                              | TAny            -> tRefWrapper(TAny), repacked_ast
                                          (* Indexing to string returns char code, see ".elem" in Language.ml *)
                                              | TString         -> tRefWrapper(TConst), repacked_ast
                                              | TArr(elem_type) -> tRefWrapper(elem_type), repacked_ast
    ...
    let check_cast_runtime ((st, i, o, s::vs) as conf) loc tt label positive =
      ...
      if not (Typecheck.Conformity.value_conforms st s tt)
